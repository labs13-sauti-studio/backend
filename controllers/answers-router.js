...
router.get('/:id', async (req, res) => {
  ...
  try {
    if (answers) {
      ...
      res
        .status(404)
        .json({ message: 'error getting Answer with that ID does not exist.' });
    }
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    const updateAnswers = await Answers.updateAnswer(req.params.id, req.body);
    if (updateAnswers)
      res.status(200).json({
        message: `Client: ${updateAnswers}`,
